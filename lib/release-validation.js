...
function enabledFlag(value) {
  ...
  throw new TypeError("JUMPGATE_UAT_MODE must be 0 or 1");
}
...
function loadReleaseValidationConfig(env = process.env) {
  const enabled = enabledFlag(env.JUMPGATE_UAT_MODE);
  if (!enabled) {
    return Object.freeze({ enabled: false, scenarios: UAT_SCENARIOS });
  }
