...
function assertStorageConfig(config) {
  ...
  if (environment === "production" && config.providerMutationMode !== "fenced") {
  ...
  assertPostgresMigrationCeiling(config.postgresMigrationCeiling, {
    required: environment === "production" && config.durableDriver === "postgres",
  });
  assertRedisPlaybackWriteVersion(config.redisPlaybackWriteVersion);
  if (environment === "production" && config.redisPlaybackWriteVersion !== "4") {
  ...
  if (environment === "production" && config.ephemeralSecurityMaterial === true) {
...
function assertProductionSubtitleErasureReadiness(config) {
  if (!config || config.environment !== "production") return true;
  if (
    config.subtitleS3 &&
...
function normalizeRedisTopologyError(error) {
  if (error && error.code === "redis_cluster_unsupported") return error;
...
async function assertRedisWritablePrimary(client, keyspace) {
  ...
  try {
    ...
    if (redisReplyString(stored, "write").toUpperCase() !== "OK") {
      throw new Error("Redis readiness write probe was not stored");
    }
    created = true;
  } finally {
    if (created) {
      const removed = Number(await client.sendCommand(["DEL", probeKey]));
...
async function runProductionStoragePreflight(config, options = {}) {
  const safeConfig = assertStorageConfig(config);
  if (
    safeConfig.environment !== "production" ||
    safeConfig.durableDriver !== "postgres" ||
    safeConfig.ttlDriver !== "redis"
  ) {
    throw new TypeError("production storage preflight requires PostgreSQL and Redis");
  }
  const expectedRedisAction = options.expectedRedisAction;
  if (expectedRedisAction !== "initialize-v5" && expectedRedisAction !== "advance-v6") {
    ...
    if (safeConfig.ttlDriver === "memory") {
      ...
      ttlHealth = {
        ...
        run: async () => {
          assertPong(await client.ping());
          try {
            if (safeConfig.environment === "production") {
