...
async function assertRedisNoEvictionPolicy(client) {
  ...
  let policy = null;
  let infoFallbackAllowed = false;
  if (Array.isArray(reply)) {
    const fields = Reflect.ownKeys(reply);
    if (reply.length === 0 && fields.length === 1 && fields[0] === "length") {
      infoFallbackAllowed = true;
    } else {
      if (
        reply.length !== 2 ||
        fields.length !== 3 ||
        ...
        fields[1] !== "1" ||
        fields[2] !== "length" ||
        redisConfigText(reply[0]) !== "maxmemory-policy"
      ) {
        throw new Error("Redis maxmemory-policy readiness reply is invalid");
      }
      policy = redisConfigText(reply[1]);
      if (policy === null) {
        throw new Error("Redis maxmemory-policy readiness reply is invalid");
      }
    }
    ...
    if (fields.length === 0) {
      ...
      if (
        ...
        (fields[0] !== "maxmemory-policy" && fields[0] !== "maxmemoryPolicy")
      ) {
        throw new Error("Redis maxmemory-policy readiness reply is invalid");
      }
      const descriptor = Object.getOwnPropertyDescriptor(reply, fields[0]);
      if (!descriptor || descriptor.enumerable !== true || !("value" in descriptor)) {
        throw new Error("Redis maxmemory-policy readiness reply is invalid");
      }
      policy = redisConfigText(descriptor.value);
      if (policy === null) {
        throw new Error("Redis maxmemory-policy readiness reply is invalid");
      }
    }
  ...
  if (infoFallbackAllowed) {
    const info = redisConfigText(await client.sendCommand(["INFO", "memory"]));
    if (info === null) {
      throw new Error("Redis maxmemory-policy readiness reply is invalid");
    }
    const values = info
      .split(/\r?\n/)
      .filter((line) => line.startsWith("maxmemory_policy:"))
      .map((line) => line.slice("maxmemory_policy:".length));
    if (values.length !== 1 || !/^[a-z][a-z0-9-]{0,63}$/.test(values[0])) {
      throw new Error("Redis maxmemory-policy readiness reply is invalid");
    }
    policy = values[0];
  }
