-- jg-script:subtitle-update-authority
...
local expectedProviderRevision = ARGV[2]
local expectedGeneration = ARGV[3]
local providerRevision = ARGV[4]
local generation = ARGV[5]
local maxAuthorities = ARGV[6]
local maxProfileArtifacts = tonumber(ARGV[7])

if type(profileTag) ~= "string" or #profileTag ~= 64 or
   string.match(profileTag, "^[a-f0-9]+$") == nil or
   #KEYS ~= 13 or #ARGV ~= 7 or
   ...
   not maxProfileArtifacts or maxProfileArtifacts < 1 or maxProfileArtifacts > 64 or
   ((expectedProviderRevision == "") ~= (expectedGeneration == "")) or
   (expectedProviderRevision ~= "" and not subtitle_valid_decimal(expectedProviderRevision, 128)) or
   (expectedGeneration ~= "" and not subtitle_valid_generation(expectedGeneration)) then
...
if not raw then
  if expectedProviderRevision ~= "" then return { "authority_conflict" } end
  ...
  if current.providerRevision == providerRevision and current.generation == generation then
    ...
    if current.providerRevision ~= expectedProviderRevision or current.generation ~= expectedGeneration then
