...
if not playback_valid_identifier(ARGV[18], true) or
   ...
   not playback_valid_digest(ARGV[26], false) or
   not playback_valid_canonical_uuid(ARGV[27]) or
   not playback_valid_digest(ARGV[28], false) or
   (ARGV[29] ~= "required" and ARGV[29] ~= "allow_missing") then
...
if not playback_valid_string_array(requestedList, 32, true) or
   #KEYS ~= 26 + #requestedList then return { "profile_collision" } end
...
local attemptFingerprintKey = KEYS[21 + #requestedList]
local globalAttempts = KEYS[22 + #requestedList]
local profileAttempts = KEYS[23 + #requestedList]
local reconcileIndex = KEYS[24 + #requestedList]
local activeAttemptKey = KEYS[25 + #requestedList]
local writerProtocolKey = KEYS[26 + #requestedList]
local activeAttemptType = playback_key_type(activeAttemptKey)
if activeAttemptType ~= "none" and activeAttemptType ~= "string" then
  return { "profile_collision" }
end
local writerProtocolType = playback_key_type(writerProtocolKey)
if writerProtocolType ~= "none" and writerProtocolType ~= "string" then
  return { "profile_collision" }
end
local writerProtocol = writerProtocolType == "string" and
  redis.call("GET", writerProtocolKey) or nil
if writerProtocol ~= "6" and
   (ARGV[29] == "required" or writerProtocol ~= nil) then
...
local attemptFingerprintType = playback_key_type(attemptFingerprintKey)
if attemptFingerprintType ~= "string" or
   redis.call("GET", attemptFingerprintKey) ~= ARGV[28] then
  return { "claim_attempt_changed" }
end
...
local activeAttempt = redis.call("GET", activeAttemptKey)
local activeAttemptOwned = not activeAttempt or activeAttempt == attemptKey
local previousMatchesAttempt = previous and
  previous.v == "4" and
  previous.authorityVersion == ARGV[21] and
  previous.intentUrlHash == ARGV[3] and
  previous.launchedAtMs == ARGV[4] and
  previous.requestDigest == ARGV[26] and
  previous.sessionId == ARGV[6]

if attempt.resultStatus ~= "pending" then
  if attempt.resultStatus ~= "claimed" then
    if attempt.resultStatus ~= "ambiguous" and attempt.resultStatus ~= "expired" and
       attempt.resultStatus ~= "not_found" then return { "claim_attempt_changed" } end
    return { "claimed", attempt.resultStatus, ARGV[6] }
  end
  if not previousMatchesAttempt or not activeAttemptOwned or
     previous.released ~= "0" or previous.status ~= "claimed" then
  ...
  local result = {
    "claimed", "claimed", previous.sessionId, contextRaw,
    previous.claimedAtMs, previous.expiresAtMs
  }
  table.insert(result, previous.privateStateEnvelope)
  return result
end

if previous and activeAttemptOwned and
   previous.intentUrlHash == ARGV[3] and previous.launchedAtMs == ARGV[4] then
  if not previousMatchesAttempt then return { "claim_request_conflict" } end
...
if #matches == 1 then
  ...
  if playback_key_type(KEYS[19]) ~= "none" then return { "session_collision" } end
  claim.status = status
...
if status == "claimed" then
  redis.call("SET", KEYS[19], keys.root)
...
redis.call("SET", activeAttemptKey, attemptKey)
redis.call("PEXPIREAT", activeAttemptKey, stateExpiresAtMs)
playback_refresh_profile_ttl(keys, now, tombstoneTtlMs)
if status == "claimed" then
  local result = {
