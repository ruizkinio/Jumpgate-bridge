...
function assertStorageTopology(environment, durableDriver, ttlDriver) {
  ...
  if (isProductionLikeEnvironment(environment) &&
      (durableDriver !== "postgres" || ttlDriver !== "redis")) {
...
function loadStorageConfig(env = process.env, options = {}) {
  ...
  const durableFallback = environment === "test" ? "memory" : productionLike ? "postgres" : "sqlite";
  const ttlFallback = productionLike ? "redis" : "memory";
